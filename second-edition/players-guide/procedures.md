...
- Le **Gardien** doit présenter les informations évidentes sur une zone et ses dangers, librement sans conséquence aucune. Se déplacer rapidement ou sans précaution peut augmenter le risque de rencontrer un monstre errant, de déclencher un piège ou un jet de dé sur la table des [**Événements du donjon**](#dungeon-events).

> Bien que le terme "donjon" soit utilisé ici, il peut désigner n'importe quel lieu dangereux (manoir, ferme, site d'aventure, etc.).

### Cycle d'exploration du donjon

1. Le **Gardien** décrit l'environnement du groupe et tout danger immédiat (combat, pièges, surprises, etc.). Les joueur⋅euses déclarent ensuite les mouvements et les **actions** voulus pour leur personnage.
...
3. Les joueur⋅euses notent toute perte de ressources et toute nouvelle condition (utilisation d'un objet, _privation_, etc.). Ensuite, le cycle recommence . Le cas échéant, le **Gardien** devrait faire un jet sur le tableau des [**Événements du donjon**](#dungeon-events). Faites preuve de bon sens lorsque vous interprétez les résultats !

### Événements du donjon

L'exploration d'un donjon est toujours dangereuse, et le temps doit toujours être mis en balance avec le risque de réveiller les habitants de l'endroit, les aléas naturels, et pire encore. Lorsque le groupe :
...
- Se déplace dans un nouvel espace, un nouveau niveau ou une nouvelle zone
- Est bruyant

**Lancez le dé sur la table ci-dessous.**
...
| **3** | **Environnement** | L'environnement se modifie ou dégénère. L'eau monte, les plafonds s'effondrent, un rituel est en passe d'être achevé, etc. |
...
- Les **actions** sont toutes les activités non passives, telles que _rechercher des pièges_, _forcer une porte_, _être à l'affût du danger_, _désamorcer un piège_, _engager le combat avec un ennemi_, _lancer un sort_, _éviter un piège_, _fuir_, _se reposer_, etc.
- Certaines **actions** ont des règles spéciales (voir ci-dessous), tandis que d'autres peuvent prendre plusieurs **tours**.
...
- Un personnage entouré d'ennemis, enveloppé par les ténèbres ou confronté à ses plus grandes peurs peut être pris de _panique_. Une **sauvegarde** de **VOL** est généralement nécessaire pour éviter de perdre le contrôle et être pris de _panique_.
- Pour surmonter son état, un personnage _paniqué_ doit faire une **sauvegarde** de **VOL** comme **action** à son **tour**.
- Un personnage _paniqué_ a 0 **PP**, n'agit pas au premier round de combat et toutes ses attaques sont _affaiblies_.

### Éléments du donjon

#### Lumière
- Les torches et autres sources de lumière radiales éclairent 12 m du donjon. Au-delà, seuls de faibles contours sont visibles. Les torches durent jusqu'à ce qu'elles soient éteintes par un personnage ou son environnement.
...
- Les personnages sans source de lumière peuvent être atteint de _panique_ jusqu'à ce que l'on aie remédié à leur situation.

#### Portes

- Les portes et les entrées peuvent être verrouillées, coincées ou totalement bloquées. Les personnages peuvent essayer de forcer une porte (ou de la bloquer) en utilisant les ressources disponibles (clous, colle) ou leur compétence naturelle.
- L'ordre de marche du groupe détermine qui est le plus touché par ce qui se trouve derrière une porte.
- Un personnage peut détecter, par une observation attentive (en écoutant, en sentant, etc.), des signes de vie et d'autres dangers à travers les portes et les murs avoisinants.

#### Pièges

- Un personnage prudent devrait recevoir toutes les informations qui lui donneraient l'opportunité d'_éviter_ de déclencher un piège. Un personnage inattentif déclenchera un piège selon la fiction, ou aura 2 chances sur 6 de le faire.
- Les **pièges** peuvent généralement être détectés en **fouillant** minutieusement une pièce.
- Les dégâts infligés par les pièges sont pris sur les Attributs (généralement **FOR** ou **DEX**) et _non_ sur les **PP**. L'armure peut réduire les dégâts, mais seulement s'il y a lieu (par exemple, un bouclier ne réduirait pas les dégâts causés par un gaz nocif).
...
- Les destinations potentielles sur une carte sont appelées des **points**.
...
| **Difficile** | **Forêts, déserts, collines** | +1 Veille | _Animaux sauvages, inondations, équipement détérioré, chutes de pierres, abris peu sûrs, pièges de chasseurs_ |
...
Chaque jour, le Gardien devrait faire un jet sur la table de météo pour la saison appropriée. Si le résultat "**Extrême**" est obtenu deux fois de suite, le temps devient "**Catastrophique**". Un grain devient un ouragan, une tempête inonde la vallée, etc.
...
| **Mauvais** | Ajoutez une **Fatigue** _ou_ une **veille** au voyage. | _Vents violents, averses, chaleur accablante, air frais_ |
...
1. Le **Gardien** décrit le **point** ou la **région** actuels sur la carte et explique comment le chemin, la météo, le terrain ou l'état du groupe peuvent influer sur la **vitesse de déplacement**. Le groupe trace ou ajuste un itinéraire vers sa destination.
2. Chaque membre du groupe choisit une seule **Action en contrées sauvages**. Le **Gardien** narre les résultats, puis lance le dé sur la table des [**Événements en contrées sauvages**](#wilderness-events). Le groupe réagit aux résultats.
3. Les **joueur⋅euses** et le **Gardien** notent les éventuelles pertes de ressources et nouvelles conditions (utilisation de torches, état de _privation_, etc.), et le cycle se répète.
...
| **2** | **Signe** | Le groupe découvre un indice, une trace ou une indication d'une rencontre, d'un endroit, d'un élément caché à proximité ou des informations à propos d'une zone proche. |
...
| **5** | **Épuisement** | Le groupe rencontre un obstacle contraignant à des efforts, à des précautions ou à des contre-temps. Cela peut signifier mettre plus de temps (et effectuer une **Action en contrées sauvages**), ou ajouter de la **Fatigue** à l'inventaire des PJ, pour représenter les difficultés rencontrées. |
...
### Éléments des contrées sauvages

#### Nuit

- Le groupe peut choisir de voyager la nuit et de se reposer la journée, mais le voyage de nuit est bien plus lent et plus périlleux !
...
- Certains terrains et certaines conditions météo peuvent être plus faciles à traverser la nuit (le désert, par exemple). Le **Gardien** devrait compenser ces difficultés ainsi que toutes les autres.

#### Dormir

- La dernière **veille** de la journée est habituellement réservée à l'action [**Dresser le camp**](#make-camp).
- Les personnages ont généralement besoin de dormir chaque jour. Toute interruption plus que mineure peut inverser ou annuler les avantages du sommeil.
...
- Une lanterne peut être rallumée indéfiniment mais nécessite un bidon d'huile à part (6 utilisations).
...
- Si le groupe est perdu, il peut avoir besoin de dépenser une **Action en contrées sauvages** pour retrouver son chemin. Autrement, le groupe atteint le **point** suivant de son itinéraire.

> N'oubliez pas de comparer les résultats de l'égarement à la **Difficulté** du chemin correspondante.

#### Explorer

- Un ou plusieurs membres du groupe fouillent une vaste zone, à la recherche d'éléments cachés, en éclaireur ou en marchant prudemment.
...
- L'expérience ou l'équipement pertinents peuvent également augmenter le butin récolté.
...
- **Ressources**: Les coûts non monétaires tels que des biens matériels, des articles courants spécifiques, etc.
...
- Un **homme sauvage** qui vit dans les profondeurs de la **Forêt des Couteaux**. Le groupe l'a rencontré lors d'une précédente expédition et la rencontre ne s'est pas bien terminée. Pourtant, on dit qu'il connaît la nature de toutes les herbes médicinales et leurs propriétés curatives.
...
Un personnage peut améliorer ses compétences à utiliser un objet ou une capacité, avec des résultats narratifs ou mécaniques clairs. Il peut vouloir infliger des dégâts plus importants avec une arme particulière, réduire ses chances de se perdre par mauvais temps ou apprendre à lire les langues des anciens. Un PJ peut passer plusieurs **Actions de temps libre** à s'entraîner avec une arme particulière, améliorant ses compétences semaine après semaine. Il peut aussi avoir besoin de se rendre chez un sage lointain, afin de s'améliorer grâce à des études courtes mais intenses.

La joueur⋅euse doit décrire précisément ce qu'iel aimerait améliorer et un **Maître** avec lequel iel pourrait s'entraîner. Et bien sûr, la motivation du personnage à s'améliorer doit venir d'une expérience en jeu. Le **Gardien** devrait fournir les **jalons** et les **coûts** associés.

**Exemples :**

- **La Parade à deux mains**: Lorsqu'un PJ se bat avec une main libre, ses PP augmentent temporairement de 1d4. Le groupe a engagé une mercenaire des **Terres Effondrées**, dont les combattants mettent l'accent sur l'évitement des attaques ennemies. Elle a accepté d'entraîner toute personne capable de la battre au corps à corps.
